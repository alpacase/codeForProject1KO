...
for fq in `ls *_1_val_1.fq`
do
filename="${fq%_1_val_1.fq}";
  echo $filename ;
  hisat2 -p 8 --dta -x ~/ngs_analysis/resource/ht2Build_mm10/mm10 \
  -1 ${filename}_1_val_1.fq \
  -2 ${filename}_2_val_2.fq \
...
for bam in `ls *.bam`; do
	name=${bam%%.*};
	bamCoverage -b $bam -o ../bigwig/${name}.bw \
	--normalizeUsing RPKM \
	-p 10 -v -bs 1
done
